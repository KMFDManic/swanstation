...
SwanStation (Darthmew is most active currently with project commits! Thanks:) is a hard fork and open-source Libretro core implementation of DuckStation (originally created and worked upon by stenzek! Appreciation:), which is an emulator of the Sony PlayStation(TM) console, focusing on playability, speed, and long-term maintainability. The goal is to be as accurate as possible while maintaining performance suitable for low-end devices. "Hack" options are discouraged, the default configuration should support all playable games with only some of the enhancements having compatibility issues.
